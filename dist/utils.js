...
exports.derivationSchemeInternalConvert = void 0;
...
const toXpub = (notXpub, version = MAINNET_XPUB) => bs58check.encode(Buffer.concat([version, bs58check.decode(notXpub).slice(4)]));
const getDerivationSchemeMatch = (scheme) => {
    ...
    if (!match)
        throw new Error('Invalid derivationScheme');
    return match;
};
const derivationSchemeLTCType = (xpubs) => {
    const xpubVersions = xpubs.map(xpub => bs58check.decode(xpub).slice(0, 4));
...
const derivationSchemeAllSameXpubType = (xpubs) => {
    const xpubVersions = xpubs.map(xpub => bs58check.decode(xpub));
    return xpubVersions.every(version => {
        return (version.length === 78 &&
            version.slice(0, 4).equals(xpubVersions[0].slice(0, 4)));
    });
};
const derivationSchemeConvertXpubs = (scheme, version = MAINNET_XPUB) => {
...
exports.derivationSchemeInternalConvert = (scheme) => {
    const [, prefix, xpubsStr] = getDerivationSchemeMatch(scheme);
    const xpubs = xpubsStr.split('-');
    if (xpubs.length > 1 && prefix === '')
        throw new Error('Invalid derivationScheme: Multiple xpubs but no "m-of-" prefix');
    if (!derivationSchemeAllSameXpubType(xpubs))
        throw new Error('Invalid derivationScheme: All xpubs should be same version');
